...
from core.security import Hasher, create_access_token, get_current_user,create_refresh_token, ALGORITHM, SECRET_KEY, REFRESH_TOKEN_EXPIRE_DAYS
from schemas import UserCreate, SignUpResponse
...
async def login(
    ...
    new_access_token = create_access_token(data={"sub": str(user.id), "name": user.name, "is_active": user.is_active})
...
async def sign_up(
    ...
    access_token = create_access_token(data={"sub": str(new_user.id), "name": new_user.name, "is_active": new_user.is_active})
